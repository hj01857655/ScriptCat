...
// @author              coder_tq
// @match               https://onlineweb.zhihuishu.com/onlinestuh5
// @match               https://studyvideoh5.zhihuishu.com/stuStudy*
// @match               https://onlineexamh5new.zhihuishu.com/stuExamWeb.html*
...
} else if (location.href.includes('dohomework')) {
    ...
    setTimeout(function () {
        if ($('.subject_describe>div>div').length) {
            ...
            $subject_describe.click(function () {
                str = this.shadowRoot.innerHTML.replace('？', '').replace(/<[^>]+>/g, '').replace(' ', '').replace('&nbsp;', '').replace(/（/, '').replace(/）/, '')
                GM_setClipboard(str)
                Swal.fire({
                    icon: 'success',
                    title: '复制成功',
...
function hookVideo(tip) {
    ...
    _self.XMLHttpRequest = setting.que ? function () {
        var ajax = new xhr(),
            open = ajax.open;
        ajax.open = function (url, _method) {
            if (url.match('/loadVideoPointerInfo')) {
                method = _method = 'OPTIONS';
            }
