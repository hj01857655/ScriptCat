// ==UserScript==
// @name                超新星学习小助手⭐视频、测验、作业、考试
// @namespace           QFLYD
// @version             1.7.8
// @description         【🥇操作简单】超新星学习小助手，无需任何配置，安装即可使用。【🔊功能齐全】支持视频、音频倍速秒过；文档、图书自动完成；字体解密；章节测验、课后作业、考试自动答题、自动提交、自动收录答案；支持自动切换任务点、自动登录等。【📔独家题库】独家丰富试题库，每日更新，精准识别，答案全对。【✨功能扩展】开放自定义参数，功能控制更简单
// @author              QFLYD
// @match               *://*.chaoxing.com/*
// @match               *://*.edu.cn/*
// @match               *://*.nbdlib.cn/*
...
if (_l.hostname == 'i.mooc.chaoxing.com' || _l.hostname == "i.chaoxing.com") {
    ...
    Swal.fire('超星学习小助手提示', '注意：专业课可能会存在答案不全或无答案，请谨慎使用脚本考试，开始考试之前请确保该账号已激活脚本。', 'info')
...
function showTips() {
    GM_xmlhttpRequest({
        ...
        ontimeout: function () {
            var _msg = "链接不到云端服务器，可能是您使用的脚本版本过低，请尽快更新，最新脚本更新发布官网：https://script.521daigua.cn";
            Swal.fire('超星学习小助手提示', _msg, 'info');
        }
    });
}
...
function missonWork(dom, obj) {
    ...
    if (isDo) {
        if (obj['jobid'] !== undefined) {
            ...
            // setTimeout(() => { startDoCyWork(0, dom) }, 1000)
            setTimeout(() => { startDoPhoneCyWork(0, dom, phoneWeb) }, 1000)
        } else {
            setTimeout(() => { startDoCyWork(0, dom) }, 1000)
        }
    } else {
...
function startDoPhoneCyWork(index, doms, phoneWeb) {
    ...
    getElement($(doms[index]).contents()[0], 'iframe').then(element => {
        ...
        if (workIframe.length == 0) {
            setTimeout(() => { startDoPhoneCyWork(index, doms) }, 1000)
        }
        ...
        if (setting.share && workStatus.indexOf("已完成") != -1) {
            ...
            getElement($(doms[index]).contents()[0], 'iframe[src="' + phoneWeb + '"]').then((element) => {
                setTimeout(() => { doPhoneWork($(element).contents()) }, 1000)
            })
        } else if (workStatus.indexOf('待批阅') != -1) {
            _mlist.splice(0, 1)
            _domList.splice(0, 1)
            logger('测验：' + (index + 1) + ',测验待批阅,跳过', 'red')
            setTimeout(() => { startDoPhoneCyWork(index + 1, doms, phoneWeb) }, 1000)
        } else {
            _mlist.splice(0, 1)
            _domList.splice(0, 1)
...
function startDoCyWork(index, doms) {
    ...
    getElement($(doms[index]).contents()[0], 'iframe').then(element => {
        let workIframe = element
        if (workIframe.length == 0) {
            setTimeout(() => { startDoCyWork(index, doms) }, 1000)
        }
        let workStatus = $(workIframe).contents().find('.CeYan .ZyTop h3 span:nth-child(1)').text().trim()
        if (!workStatus) {
            _domList.splice(0, 1)
        ...
        if (setting.share && workStatus.indexOf("已完成") != -1) {
            ...
            _domList.splice(0, 1)
            logger('测验：' + (index + 1) + ',测验待批阅,跳过', 'red')
            setTimeout(() => { startDoCyWork(index + 1, doms) }, 1000)
        } else {
            _mlist.splice(0, 1)
            _domList.splice(0, 1)
            logger('测验：' + (index + 1) + ',未知状态或用户选择不收录答案,跳过', 'red')
